...
export const addQuestoQb = (req, res, next) => {
  ...
  if (qbId && quesId) {
    qBank.update({ _id: qbId }, { $push: { questions: quesId } }).exec((err, qb) => {
      if (err) {
        return res.json({ success: false, message: 'Something going wrong', err });
      } else {
        next();
...
export const updateQbank = (req, res, next) => {
  ...
  if (req.body) {
    ...
    qBank.findOneAndUpdate({ _id: id }, { $set: req.body }, { 'new': true }).populate('author').exec((err, qb) => {
      if (err) {
        return res.json({ success: false, message: 'Something going wrong', err });
      } else {
        return res.json({
...
export const searchQbanks = (req, res) => {
  ...
  if (q && q !== '') {
    qBank.find({ $or: [{ title: regex }, { summary: regex }] }).limit(20).exec((err, qb) => {
      if (err) {
        return res.json({ success: false, message: 'Something going wrong' });
      } else {
        if (!qb || qb === null) {
