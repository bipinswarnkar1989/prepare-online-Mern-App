...
export const loginWithSocial = (req, res) => {
  console.log(req.body);
...
export const addBookMarkInUser = (req, res, next) => {
  ...
  if (bM) {
    User.updateOne({ _id: user }, { $push: { bookMarks: bM } }).exec((err, result) => {
      if (err) {
        return res.json({ success: false, message: 'Something going wrong' });
      } else {
        return res.json({
...
export const removeBookMarkFromUser = (req, res, next) => {
  ...
  if (bM && user) {
    User.updateOne({ _id: user }, { $pull: { bookMarks: bM } }).exec((err, result) => {
      if (err) {
        return res.json({ success: false, message: 'Something going wrong' });
      } else {
